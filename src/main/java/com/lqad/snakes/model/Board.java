...
public class Board {
    ...
    public int resolvePosition(int currentPosition, int move) {
        int winOrPelanty = currentPosition + move;
        
        if (winOrPelanty > FINAL_POSITION) {
            // Calculate how far past 100 we went
            int past100 = winOrPelanty - FINAL_POSITION;
            
            // Bounce back
            int landPosition = FINAL_POSITION - past100;
            
            // Check if the landing spot has a snake or ladder
            return checkJump(landPosition);
        }
        
        return checkJump(winOrPelanty);
    }
